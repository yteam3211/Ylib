...
public class SwerveConstants {
    ...
    public static class ModuleConfig {
        ...
        private PIDFeedForwordGains SteerMotorGains;
        private PIDFeedForwordGains DriveMotorGains;
        ...
        public ModuleConfig WithSteerMotorGains(PIDFeedForwordGains SteerMotorGains) {
        this.SteerMotorGains = SteerMotorGains;
        return this;
        }
        
        public ModuleConfig WithDriveMotorGains(PIDFeedForwordGains DriveMotorGains) {
        this.DriveMotorGains = DriveMotorGains;
        return this;
        }
        public ModuleConfig WithSteerMotorGains(Slot0Configs SteerMotorSlot0Config) {
        this.SteerMotorGains = PIDFeedForwordGains.FromSlot0Config(SteerMotorSlot0Config);
        return this;
        }
        public ModuleConfig WithDriveMotorGains(Slot0Configs DriveMotorSlot0Config) {
        this.DriveMotorGains = PIDFeedForwordGains.FromSlot0Config(DriveMotorSlot0Config);
        return this;
        }
        ...
        public static ModuleConfig fromTunerModuleConfig(SwerveModuleConstants<TalonFXConfiguration,TalonFXConfiguration,CANcoderConfiguration> constants,String name){
            return 
                new ModuleConfig(name)
                    .WithEncoderID(constants.EncoderId).WithEncoderInverted(constants.EncoderInverted).WithEncoderOffset(constants.EncoderOffset)
                    .WithDriveId(constants.DriveMotorId).WithDriveMotorInverted(constants.DriveMotorInverted).WithDriveMotorGains(constants.DriveMotorGains).WithDriveMotorGearRatio(constants.DriveMotorGearRatio).WithDriveFirctionVoltage(constants.DriveFrictionVoltage).WithDriveInertia(constants.DriveInertia)
                    .WithSteerId(constants.SteerMotorId).WithSteerMotorInverted(constants.SteerMotorInverted).WithSteerMotorGains(constants.SteerMotorGains).WithSteerMotorGearRatio(constants.SteerMotorGearRatio).WithSteerFirctionVoltage(constants.SteerFrictionVoltage).WithSteerInertia(constants.SteerInertia)
                    .WithCouplingGearRatio(constants.CouplingGearRatio)
                    .WithLocationX(constants.LocationX).WithLocationY(constants.LocationY)
                    .WithSpeedAt12Volts(constants.SpeedAt12Volts).WithSlipCurrent(constants.SlipCurrent)
                    .WithWheelRadius(constants.WheelRadius);
        }
        public boolean isValid(){
            return CanBus != null && DriveId != 0 && SteerId != 0 && EncoderID != 0 && 
            LocationX != 0 && LocationY != 0 && 
            SpeedAt12Volts != 0 && SlipCurrent != 0 && 
            DriveMotorGains != null && SteerMotorGains != null &&
            WheelRadiusMeter != 0 && 
            DriveMotorGearRatio != 0 && SteerMotorGearRatio != 0;
        }
    }
