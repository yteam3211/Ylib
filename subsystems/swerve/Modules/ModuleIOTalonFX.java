...
public class ModuleIOTalonFX implements ModuleIO {
  ...
  public ModuleIOTalonFX(SwerveConstants.ModuleConfig config) {
    if (config.isValid()) {
      throw new IllegalArgumentException(
          "the module "
              + config.getName()
              + "Config is not Valid and will not work check if all the constantst are right");
    }
    this.config = config;
    CANBus canBus = new CANBus(config.getCanBus());
    /**
     * config of the Drive motor start with break mode for the drive motor then inverted then gains
    ...
    steerConfiguration.MotorOutput.Inverted =
        config.isSteerMotorInverted()
            ? InvertedValue.CounterClockwise_Positive
            : InvertedValue.Clockwise_Positive;
    ...
    steerConfiguration.Feedback.FeedbackSensorSource =
        SteerMotor.getIsProLicensed().getValue()
            ? FeedbackSensorSourceValue.FusedCANcoder
            : FeedbackSensorSourceValue.RemoteCANcoder;
    steerConfiguration.Feedback.RotorToSensorRatio = config.getSteerMotorGearRatio();
    steerConfiguration.MotionMagic.MotionMagicCruiseVelocity =
        100.0 / config.getSteerMotorGearRatio();
    steerConfiguration.MotionMagic.MotionMagicAcceleration =
        steerConfiguration.MotionMagic.MotionMagicCruiseVelocity / 0.100;
    ...
    SteerPositionQueue =
        PhoenixOdometryThread.getInstance().registerSignal(SteerMotor.getPosition());
  }
  ...
  public void runState(SwerveModuleState state) {
    ...
    state.cosineScale(Rotation2d.fromRotations(CanCoder.getAbsolutePosition().getValueAsDouble()));
  ...
  public void UpdateInputs(ModuleInputs inputs) {
    ...
    inputs.odometryDrivePositionsRot =
        DrivePositionQueue.stream().mapToDouble((Double val) -> val).toArray();
    inputs.odometrySteerPositions =
        SteerPositionQueue.stream()
            .map((Double val) -> Rotation2d.fromRotations(val))
