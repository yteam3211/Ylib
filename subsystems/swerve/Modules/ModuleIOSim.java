  ...
  public ModuleIOSim(ModuleConfig config, DCMotor DriveMotor, DCMotor SteerMotor) {
    ...
    positionController =
        new ProfiledPIDController(
            config.getSteerMotorGains().getK_P(),
            config.getSteerMotorGains().getK_I(),
            config.getSteerMotorGains().getK_D(),
            new TrapezoidProfile.Constraints(
                SteerMotor.withReduction(config.getSteerMotorGearRatio()).freeSpeedRadPerSec,
                SteerMotor.withReduction(config.getSteerMotorGearRatio()).freeSpeedRadPerSec
                    / 0.100));
    positionController.enableContinuousInput(-0.5, 0.5);
    VelocityController =
        new PIDController(
            config.getDriveMotorGains().getK_P(),
            config.getDriveMotorGains().getK_I(),
            config.getDriveMotorGains().getK_D());
    VelocityFeedforward =
        new SimpleMotorFeedforward(
            config.getDriveMotorGains().getK_S(),
            config.getDriveMotorGains().getK_V(),
            config.getDriveMotorGains().getK_A());
    SteerSim =
        new DCMotorSim(
            LinearSystemId.createDCMotorSystem(
                SteerMotor, config.getSteerInertia(), config.getSteerMotorGearRatio()),
            SteerMotor,
            0.01,
            0.08);
    DriveSim =
        new DCMotorSim(
            LinearSystemId.createDCMotorSystem(
                DriveMotor, config.getDriveInertia(), config.getDriveMotorGearRatio()),
            DriveMotor,
            0.03,
            0.13);
  }
  ...
  private void UpdateSim() {
    if (VelocityClosedLoop) {
      DriveSim.setInputVoltage(
          VelocityFeedforward.calculate(VelocityTarget)
              + VelocityController.calculate(
                  DriveSim.getAngularVelocityRPM() / 60, VelocityTarget));
    }
    DriveSim.update(0.02);
    if (postionClosedLoop) {
      SteerSim.setInputVoltage(
          positionController.calculate(
              SteerSim.getAngularPositionRotations(), PostionTarget.getRotations()));
    }
